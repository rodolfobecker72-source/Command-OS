...
-- Criadas ANTES das tabelas porque as RLS policies dependem delas.
-- Referências circulares são resolvidas porque as funções são STABLE
-- e só executam em runtime, quando as tabelas já existem.
-- ============================================================================
...
CREATE TABLE public.profiles (
  id UUID PRIMARY KEY,  -- referencia auth.users(id) logicamente, sem FK explícita
...
-- 3.5 workspace_settings (1:1 com workspace)
...
-- 3.6 workspace_layout (1:1 com workspace)
...
-- 3.23 landing_leads (pública, sem workspace_id)
...
CREATE POLICY workspace_members_insert_admin ON public.workspace_members FOR INSERT TO authenticated
  WITH CHECK (has_workspace_access(auth.uid(), workspace_id) AND get_user_role(auth.uid()) = ANY(ARRAY['owner'::app_role, 'admin'::app_role]));
CREATE POLICY workspace_members_update ON public.workspace_members FOR UPDATE TO authenticated
  USING (has_workspace_access(auth.uid(), workspace_id) AND get_user_role(auth.uid()) = ANY(ARRAY['owner'::app_role, 'admin'::app_role]));
...
-- ---- score_history (sem UPDATE — intencional) ----
...
-- ---- monthly_goals (write restrito a owner) ----
...
CREATE POLICY monthly_goals_insert ON public.monthly_goals FOR INSERT TO authenticated
  WITH CHECK (has_workspace_access(auth.uid(), workspace_id) AND get_user_role(auth.uid()) = 'owner');
CREATE POLICY monthly_goals_update ON public.monthly_goals FOR UPDATE TO authenticated
  USING (has_workspace_access(auth.uid(), workspace_id) AND get_user_role(auth.uid()) = 'owner');
CREATE POLICY monthly_goals_delete ON public.monthly_goals FOR DELETE TO authenticated
  USING (has_workspace_access(auth.uid(), workspace_id) AND get_user_role(auth.uid()) = 'owner');

-- ---- landing_leads (pública) ----
...
-- Trigger: atualizar updated_at em workspace_layout
...
CREATE INDEX IF NOT EXISTS idx_hard_drives_workspace_id ON public.hard_drives(workspace_id);
CREATE INDEX IF NOT EXISTS idx_legacy_projects_workspace_id ON public.legacy_projects(workspace_id);
...
-- ============================================================================
-- 7. STORAGE BUCKETS
-- Executar via SQL. Para políticas de storage, usar o Dashboard:
...
-- Storage policies: leitura pública
CREATE POLICY "Public read avatars" ON storage.objects FOR SELECT TO public
  USING (bucket_id = 'avatars');
CREATE POLICY "Public read logos" ON storage.objects FOR SELECT TO public
  USING (bucket_id = 'logos');

-- Storage policies: upload/update/delete para autenticados
...
CREATE POLICY "Auth update logos" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'logos');
CREATE POLICY "Auth delete logos" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'logos');
...
-- ============================================================================
-- Execute estas queries para validar que tudo foi criado corretamente:
--
-- 1. Verificar enum:
--    SELECT unnest(enum_range(NULL::app_role));
--    → Deve retornar: owner, admin, vendedor, visualizador
--
-- 2. Contar tabelas:
...
--    → Deve retornar: 23
--
-- 3. Verificar RLS ativo em todas:
...
--    → Deve retornar: ~88 policies
--
-- 5. Verificar funções:
--    SELECT routine_name FROM information_schema.routines
--    WHERE routine_schema = 'public' ORDER BY routine_name;
--    → Deve conter: accept_invite, get_user_role, get_user_workspace,
--      handle_new_user, handle_signup_workspace, has_role,
--      has_workspace_access, update_updated_at_column
--
-- 6. Verificar trigger handle_new_user:
--    SELECT trigger_name, event_object_table FROM information_schema.triggers
--    WHERE trigger_schema = 'auth';
--    → Deve conter: on_auth_user_created em users
--
-- 7. Verificar storage buckets:
--    SELECT id, name, public FROM storage.buckets;
--    → Deve conter: avatars (public=true), logos (public=true)
--
-- 8. Teste funcional: criar um usuário via Auth → Dashboard → Users → Add User
--    Depois verificar:
--    SELECT * FROM public.profiles;
--    → Deve ter criado o profile automaticamente
--
-- ============================================================================
-- CONFIGURAÇÕES ADICIONAIS (fazer manualmente no Dashboard):
-- ============================================================================
...
--    - SUPABASE_SERVICE_ROLE_KEY: (copiar do Dashboard → Settings → API)
--    - SUPABASE_ANON_KEY: (copiar do Dashboard → Settings → API)
--
