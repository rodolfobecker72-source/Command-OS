...
-- The issue: all policies were created as RESTRICTIVE, which means PostgreSQL denies everything
...
CREATE POLICY "Workspace members can view peer profiles" ON public.profiles FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM workspace_members wm1 JOIN workspace_members wm2 ON wm1.workspace_id = wm2.workspace_id WHERE wm1.user_id = auth.uid() AND wm2.user_id = profiles.id));
...
CREATE POLICY "Owner/admin can create invites" ON public.workspace_invites FOR INSERT TO authenticated WITH CHECK (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
CREATE POLICY "Owner/admin can update invites" ON public.workspace_invites FOR UPDATE TO authenticated USING (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
CREATE POLICY "Owner/admin can delete invites" ON public.workspace_invites FOR DELETE TO authenticated USING (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
...
CREATE POLICY "Owner/admin can insert members" ON public.workspace_members FOR INSERT TO authenticated WITH CHECK (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
CREATE POLICY "Owner/admin can update members" ON public.workspace_members FOR UPDATE TO authenticated USING (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
CREATE POLICY "Owner/admin can delete members" ON public.workspace_members FOR DELETE TO authenticated USING (has_workspace_access(auth.uid(), workspace_id) AND (get_user_role(auth.uid()) = ANY (ARRAY['owner'::app_role, 'admin'::app_role])));
CREATE POLICY "Users can insert themselves as owner" ON public.workspace_members FOR INSERT TO authenticated WITH CHECK ((auth.uid() = user_id) AND (role = 'owner'::app_role));
