...
export function ServiceCategories() {
  ...
  const handleAddCategory = () => {
    ...
    if (serviceCategories.some(c => c.key === key)) {
  ...
  const handleUpdateObjective = () => {
    if (!editingObjective || !editingObjective.label.trim()) {
      toast.error('O nome do objetivo não pode estar vazio');
      return;
    }
  ...
  const getObjectivesFor = (categoryKey: string) =>
    serviceObjectives
      .filter(o => o.categoryKey === categoryKey)
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header
        ...
        subtitle="Gerencie as categorias e objetivos dos serviços"
      />

      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <AnimatePresence mode="popLayout">
          {sortedCategories.map((category, catIndex) => {
            const objectives = getObjectivesFor(category.key);
                  ...
                  <CardContent className="space-y-3">
                          ...
                          {editingObjective?.id === objective.id ? (
                            <div className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  setEditingObjective({ ...editingObjective, label: e.target.value })
                                }
                                onKeyDown={(e) => {
                                ...
                                className="h-8"
                              />
                              <Button size="sm" variant="ghost" onClick={handleUpdateObjective}>
                                Salvar
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => setEditingObjective(null)}>
                                Cancelar
                              </Button>
                            </div>
                          ) : (
                    ...
                    {addingObjectiveTo === category.key ? (
                      <div className="flex items-center gap-3 p-3 bg-primary/5 rounded-lg border-2 border-dashed border-primary/20">
                        ...
                        <Button size="sm" onClick={() => handleAddObjective(category.key)}>
                          Adicionar
                        </Button>
                        <Button
                          size="sm"
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setAddingObjectiveTo(category.key);
                          setNewObjectiveLabel('');
                        }}
                      >
      ...
      <AlertDialog open={!!deleteCategoryId} onOpenChange={() => setDeleteCategoryId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir esta categoria? Todos os objetivos associados
